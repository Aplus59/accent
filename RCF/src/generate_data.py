import os
import pandas as pd
import sys

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')))

from RCF.src.Utilis import get_share_attributes
from RCF.src.moive_loader import movie_loader   # sau này bạn sẽ sửa loader riêng


def generate_interaction_data():
    """RCF - vẫn giữ nguyên original_id"""
    data_dir = os.path.join(os.path.dirname(__file__), '../data/')
    ratings_path = os.path.join(data_dir, 'ratings.dat')

    data = pd.read_csv(ratings_path, sep='::', engine='python',
                       names=['user', 'item', 'rating', 'timestamp'],
                       dtype={'user': int, 'item': int, 'rating': int, 'timestamp': int})

    loader = movie_loader()
    train_data = {'user': [], 'pos_item': [], 'neg_item': []}
    test_data = {'user': [], 'pos_item': [], 'neg_item': []}

    for user in data['user'].unique():
        user_data = data[data['user'] == user].sort_values(by=['timestamp'])
        pos_items = user_data['item'][user_data['rating'] >= 3]
        neg_items = user_data['item'][user_data['rating'] < 3]

        pos_items = list(filter(lambda x: loader.movie_dict.get(x) is not None, pos_items))
        neg_items = list(filter(lambda x: loader.movie_dict.get(x) is not None, neg_items))

        if len(neg_items) < 10 or len(pos_items) < 10:
            continue

        n_pos_tests = len(pos_items) // 5
        n_neg_tests = len(neg_items) // 5
        used = {}

        for pos_item in pos_items[:-n_pos_tests]:
            m1 = loader.movie_dict.get(pos_item)
            best_neg_item = best_shared_type = best_shared_tag = best_used = -1

            for neg_item in neg_items[:-n_neg_tests]:
                m2 = loader.movie_dict.get(neg_item)
                shared_genre, shared_director, shared_actor = get_share_attributes(m1, m2)
                shared_tag = len(shared_genre) + len(shared_director) + len(shared_actor)
                shared_type = (len(shared_genre) > 0) + (len(shared_director) > 0) + (len(shared_actor) > 0)

                if (shared_type > best_shared_type) or \
                   (shared_type == best_shared_type and shared_tag > best_shared_tag) or \
                   (shared_type == best_shared_type and shared_tag == best_shared_tag and used.get(neg_item, 0) < best_used):
                    best_neg_item = neg_item
                    best_shared_type = shared_type
                    best_shared_tag = shared_tag
                    best_used = used.get(neg_item, 0)

            if best_neg_item != -1:
                train_data['user'].append(user)
                train_data['pos_item'].append(pos_item)
                train_data['neg_item'].append(best_neg_item)
                used[best_neg_item] = used.get(best_neg_item, 0) + 1

        if len(used) > 0:
            for i in range(n_pos_tests):
                for j in range(n_neg_tests):
                    test_data['user'].append(user)
                    test_data['pos_item'].append(pos_items[-i-1])
                    test_data['neg_item'].append(neg_items[-j-1])

    pd.DataFrame(train_data).to_csv(os.path.join(data_dir, 'train.csv'), index=False)
    pd.DataFrame(test_data).to_csv(os.path.join(data_dir, 'test.csv'), index=False)


def compress(series):
    values = set()
    for s in series:
        values.update(s.unique())
    raw_id = sorted(list(values))
    compressed_id = {raw_id[i]: i for i in range(len(raw_id))}

    res = [compressed_id, raw_id]
    for s in series:
        res.append(s.map(compressed_id))
    return res


def generate_interaction_data_ncf():
    """NCF - nén ID + tạo mapping"""
    data_dir = os.path.join(os.path.dirname(__file__), '../data/')
    ratings_path = os.path.join(data_dir, 'ratings.dat')

    data = pd.read_csv(ratings_path, sep='::', engine='python',
                       names=['user', 'item', 'rating', 'timestamp'],
                       dtype={'user': int, 'item': int, 'rating': int, 'timestamp': int})

    loader = movie_loader()
    train_data = []

    for user in data['user'].unique():
        user_data = data[data['user'] == user].sort_values(by=['timestamp'])
        pos_items = user_data['item'][user_data['rating'] >= 3]
        neg_items = user_data['item'][user_data['rating'] < 3]

        pos_items = list(filter(lambda x: loader.movie_dict.get(x) is not None, pos_items))
        neg_items = list(filter(lambda x: loader.movie_dict.get(x) is not None, neg_items))

        if len(neg_items) < 10 or len(pos_items) < 10:
            continue

        n_pos_tests = len(pos_items) // 5
        n_neg_tests = len(neg_items) // 5
        all_items = set(pos_items[:-n_pos_tests]) | set(neg_items[:-n_neg_tests])
        user_data = user_data[user_data['item'].isin(all_items)]
        train_data.append(user_data)

    train_data = pd.concat(train_data)

    # ==================== TẠO MAPPING (giữ nguyên như code cũ) ====================
    original_items = sorted(train_data['item'].unique())
    item_map = {old: new for new, old in enumerate(original_items)}

    pd.DataFrame({
        'original_item': list(item_map.keys()),
        'new_item': list(item_map.values())
    }).to_csv(os.path.join(data_dir, 'item_mapping.csv'), index=False)   # lưu cùng thư mục data
    # =============================================================================

    # Nén ID
    _, _, train_data['user'] = compress([train_data['user']])
    _, _, train_data['item'] = compress([train_data['item']])

    train_data.to_csv('movielens_train.tsv', index=False, sep='\t', header=False)


if __name__ == "__main__":
    generate_interaction_data()
    generate_interaction_data_ncf()
    print("✅ Đã tạo: train.csv, test.csv, item_mapping.csv, movielens_train.tsv")